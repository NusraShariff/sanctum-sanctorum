...
def create_loan(db: Session, data: LoanCreate, now: datetime) -> LoanOut:
    ...
    active_loans = [loan for loan in member_loans if loan.returned_at is None]
    ...
    if any(loan.book_id == book.id for loan in active_loans):
        raise HTTPException(status_code=409, detail="Member already has this book")
...
def list_member_loans(
    ...
    return [to_loan_out(loan, now) for loan in loans if status is None or loan_status(loan, now) == status]
