...
def list_members(db: Session, limit: int, offset: int) -> MemberPage:
    """Return members ordered by id with total count and pagination metadata."""
    ...
    return MemberPage(items=members, total=total or 0, limit=limit, offset=offset)
...
def get_member_stats(db: Session, member_id: int, now: datetime) -> MemberStats:
    ...
    overdue_loans = sum(loan.returned_at is None and now > loan.due_at for loan in loans)
